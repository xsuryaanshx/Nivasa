...
export function StatCard({ label, value, icon: Icon, money, suffix, delta, trend = "up", delay = 0, onClick }: Props) {
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className={cn(
        "group relative overflow-hidden rounded-2xl border border-border bg-card p-3.5 sm:p-5 shadow-soft transition-all",
        onClick && "cursor-pointer hover:shadow-elev hover:border-brand/30"
      )}
    >
